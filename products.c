...
struct dynarray* create_product_array(int num_products, char** names, int* inventory, float* prices) {
    ...
    {
	struct product* temp = create_product(names[x], inventory[x], prices[x]);
        dynarray_insert(products, x, temp);
    }
...
void free_product_array(struct dynarray* products) {
	...
	for(int x = 0; x < dynarray_length(products); x++)
	{
		dynarray_free(dynarray_get(products,x));
	}
...
void print_products(struct dynarray* products) {
    ...
    for (int x = 0; x < dynarray_length(products); x++)
    {
        struct product* temp = dynarray_get(products, x);
...
struct product* find_max_price(struct dynarray* products) {
    ...
    for(int x = 0; x < dynarray_length(products); x++)
    {
        ptr_price = dynarray_get(products, x);
...
struct product* find_max_investment(struct dynarray* products) {
  ...
  //  int length_inventory = 8;
    float max_inventory = 0;

    for (int x = 0; x < dynarray_length(products); x++)
    {
        ptr_inventory = dynarray_get(products, x);
        if (max_inventory < ptr_inventory->inventory)
        {
            max_inventory = ptr_inventory->inventory;
        }
    }
...
struct product* spot = ((struct product*)dynarray_get(products, 0));
for(int x = 0; x < dynarray_length(products)-1; x++)
{
	for(int x = 0; x < dynarray_length(products)-1; x++)
	{
		if(((struct product*)dynarray_get(products, x))->inventory > ((struct product*)dynarray_get(products, x+1))->inventory)
		{
			spot = ((struct product*)dynarray_get(products, x));
			dynarray_set(products, x, ((struct product*)dynarray_get(products, x+1)));
			dynarray_set(products, x+1, spot);
		}
	}
}
}
