...
int bst_count(struct bst_node* root)
{
    ...
    if ((root->right) != NULL)
    {
        size = bst_count(root->right) + size;
    }
    
    if ((root->left) != NULL)
    {
        size = bst_count(root->left) + size;
    }
...
int bst_size(struct bst* bst) 
{
  ...
  if (bst->root != NULL)
  {
      total = bst_count(bst->root);
  }
  return total;
}

int bst_count_height(struct bst_node* root)
{
    int r;
    int l;
    ...
    r = bst_count_height(root->right);
    l = bst_count_height(root->left);
...
int path_bst(int sum, struct bst_node* root)
{
    int num;
    int temporary;
    temporary = sum - root->val;
    num = 0;

    if (root != NULL)
    {
        if (root->left == NULL && temporary == num && root->right == NULL)
        {
            return 1;
        }
        if (root->right != NULL)
        {
            num = path_bst(temporary, root->right) || num;
        }
        if (root->left != NULL)
        {
            num = path_bst(temporary, root->left) || num;
        }
            return num;    
    }
    else
